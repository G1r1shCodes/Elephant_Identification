...
class HeadCropDataset(Dataset):
    def __init__(
        self, root_dir, transform=None, min_images=3, split="train", split_ratio=0.8
    ):
        self.samples = []
        ...
        BAD_CLASSES = {
            ...
            "Herd_2_AF_ELE_15",
            "Herd_3_AF_ELE_3",
            # v8 Identity filter: min_sim < 0.20 or mean_sim < 0.55
            # Use: `if min_sim < 0.20 OR mean_sim < 0.55: REMOVE`
            "Herd_2_JM_ELE_50_D",
            "Herd_4_AF_ELE_6",
            "Herd_3_JU_ELE_36",
            "Herd_2_JM_ELE_49_R",
        ...
        target = (
            train_ids
            if split == "train"
            else (eval_ids if split == "eval" else valid_identities)
        )
...
# v8: SAFE minimal augmentation â€” nudge, don't destroy
train_transform = transforms.Compose(
    [
        transforms.Resize((256, 256)),
        transforms.RandomResizedCrop(224, scale=(0.9, 1.0)),  # conservative crop jitter
        transforms.RandomHorizontalFlip(p=0.5),  # elephants are symmetric enough
        transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        transforms.RandomErasing(
            p=0.1, scale=(0.02, 0.04)
        ),  # tiny patches only â€” don't destroy ear
    ]
)
...
DATASET_PATH = Path(
    "data/training_heads_v6"
)

print(f"Dataset path: {DATASET_PATH}")
train_dataset = HeadCropDataset(
    str(DATASET_PATH), train_transform, min_images=3, split="train"
)
eval_dataset = HeadCropDataset(
    str(DATASET_PATH), eval_transform, min_images=3, split="eval"
)
full_eval_dataset = HeadCropDataset(
    str(DATASET_PATH), eval_transform, min_images=3, split="all"
)
...
model = HeadEmbeddingModel(embed_dim=128).to(device)
arcface_head = ArcFaceHead(embed_dim=128, num_classes=train_dataset.num_classes).to(
    device
)
...
class MultiSimilarityLossV3(nn.Module):
...
class HardPositiveAlignmentLoss(nn.Module):
    """Forces anchor to align with its HARDEST positive(s) to collapse variance.
    ...
    For each anchor:
      1. Find its hardest positive(s) (lowest cosine sim, same identity)
      2. Penalize F.relu(margin - similarity)

    This forces the model to embed ALL views of the same elephant nearby,
    including the unusual pose where the ear tear is visible, without oversaturating.
    """

    def __init__(self, k=2, margin=0.7):
        super().__init__()
        self.k = k
        self.margin = margin
    ...
    def forward(self, embeddings, labels):
        ...
        for i in range(len(embeddings)):
            ...
            k = min(self.k, len(pos_sims))
            hard_pos = torch.topk(pos_sims, k=k, largest=False).values

            total_hardest_pos += hard_pos.min().item()

            # Penalize if hardest positive is far away (with margin)
            l = F.relu(self.margin - hard_pos).mean()
            loss = loss + l
            valid += 1
...
class GlobalBatchHardTripletLoss(nn.Module):
    def __init__(self, margin=0.50, memory_size=2048, embed_dim=128):
        super().__init__()
        self.margin = margin
        self.memory_size = memory_size
        self.register_buffer("memory_emb", torch.zeros(memory_size, embed_dim))
        self.register_buffer(
            "memory_labels", torch.zeros(memory_size, dtype=torch.long) - 1
        )
        self.ptr = 0
    ...
    def forward(self, embeddings, labels):
        ...
        with torch.no_grad():
            if self.ptr + batch_size <= self.memory_size:
                ...
                self.memory_emb[self.ptr :] = embeddings.detach()[
                    : batch_size - overflow
                ]
                self.memory_labels[self.ptr :] = labels.detach()[
                    : batch_size - overflow
                ]
                self.memory_emb[:overflow] = embeddings.detach()[
                    batch_size - overflow :
                ]
...
ARCFACE_LOSS_WEIGHT = 0.1
TRIPLET_LOSS_WEIGHT = 0.15
HARD_POS_WEIGHT = 1.4
MIN_SIM_WEIGHT = 0.0
BATCH_CENTER_WEIGHT = 0.6


class MinSimLoss(nn.Module):
    def __init__(self, margin=0.55):
        super().__init__()
        self.margin = margin
    ...
    def forward(self, embeddings, labels):
        ...
        for i in range(len(embeddings)):
            ...
            same_mask[i] = False
            same_sims = sim[i][same_mask]

            if len(same_sims) == 0:
                continue

            min_sim = same_sims.min()
            loss += F.relu(self.margin - min_sim)
            count += 1

        return loss / max(count, 1)


class BatchCenterPullLoss(nn.Module):
    def forward(self, embeddings, labels):
        loss = 0
        unique_labels = labels.unique()
        for lbl in unique_labels:
...
criterion_batch_center = BatchCenterPullLoss()
criterion_hard_pos = HardPositiveAlignmentLoss()
criterion_center = IdentityCenterLoss()
criterion_triplet = GlobalBatchHardTripletLoss(
    margin=0.50, memory_size=2048, embed_dim=128
).to(device)
criterion_min_sim = MinSimLoss(margin=0.55)

print("Loss 1: MS-Loss v3 (alpha=2.0, beta=75, hard_neg_k=20, T=0.2)")
print(f"Loss 2: Hard-Positive Alignment (weight={HARD_POS_WEIGHT}, k=2, margin=0.7)")
...
print(
    f"Loss 5: Global BatchHard Triplet (weight={TRIPLET_LOSS_WEIGHT}, margin=0.50, memory=2048)"
)
...
class DiversityPxMSampler:
    def __init__(self, dataset, model, device, P=8, M=3, refresh_interval=3):
        ...
        self.available_labels = [
            lbl for lbl, idxs in self.label_to_indices.items() if len(idxs) >= M
        ]
    ...
    def sample_batch(self, epoch=None):
        selected = random.sample(
            self.available_labels, min(self.P, len(self.available_labels))
        )
        batch_indices = []
        for l in selected:
            ...
            if random.random() < 0.4:
                ...
                # Random sampling within identity (30%) to stabilize collapse
                batch_indices.extend(random.sample(indices, min(self.M, len(indices))))
...
sampler = DiversityPxMSampler(train_dataset, model, device, P=10, M=3)
print(
    f"Sampler: P=10, M=3 (batch={10 * 3}), {len(sampler.available_labels)} identities"
)
...
for epoch in range(1, TOTAL_EPOCHS + 1):
    ...
    model.train()
    arcface_head.train()
    total_loss = total_ms = total_hp = total_center = total_arc = total_trip = (
        total_hard_sim
    ) = total_same_sim = 0
    n_batches = 0
    ...
    for step in range(steps_per_epoch):
        ...
        with torch.no_grad():
            ...
            same_sims = sim_matrix[same_mask].detach().cpu().numpy()
            mean_same_sim = same_sims.mean()
        ms_loss = criterion_ms(embeddings, labels)
        hard_pos_loss, batch_hard_sim = criterion_hard_pos(embeddings, labels)
        batch_center_loss = criterion_batch_center(embeddings, labels)
        ...
        min_sim_loss = criterion_min_sim(embeddings, labels)

        loss = (
            MS_LOSS_WEIGHT * ms_loss
            + HARD_POS_WEIGHT * hard_pos_loss
            + BATCH_CENTER_WEIGHT * batch_center_loss
            + CENTER_LOSS_WEIGHT * center_loss
            + ARCFACE_LOSS_WEIGHT * arc_loss
            + TRIPLET_LOSS_WEIGHT * triplet_loss
            + MIN_SIM_WEIGHT * min_sim_loss
        )

        optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(
            list(model.parameters()) + list(arcface_head.parameters()), 1.0
        )
        optimizer.step()
    ...
    history["mean_same_sim"].append(avg(total_same_sim))
    history["phase"].append(phase)

    lr = optimizer.param_groups[0]["lr"]
    print(
        f"Epoch {epoch:2d}/{TOTAL_EPOCHS} [{phase:6s}] "
        f"Loss={avg(total_loss):.4f} SameSim={avg(total_same_sim):.3f} HardSim={avg(total_hard_sim):.3f} "
...
axes[0, 1].plot(
    history["epoch"], history["ms_loss"], "b-o", markersize=3, label="MS Loss"
)
axes[0, 1].set_title("MS Loss")

axes[0, 2].plot(
    history["epoch"], history["hard_pos_loss"], "r-o", markersize=3, label="Hard-Pos"
)
axes[0, 2].set_title("Hard-Positive Alignment")

axes[1, 0].plot(
    history["epoch"], history["center_loss"], "g-o", markersize=3, label="Center"
)
axes[1, 0].set_title("Center Loss")

axes[1, 1].plot(
    history["epoch"], history["arcface_loss"], "m-o", markersize=3, label="ArcFace"
)
axes[1, 1].set_title("ArcFace Loss")

axes[1, 2].plot(
    history["epoch"], history["triplet_loss"], "c-o", markersize=3, label="Triplet"
)
...
print(f"=======================================================")
print(
    f"ZERO-SHOT EVALUATION ({len(eval_dataset.samples)} imgs, {eval_dataset.num_classes} identities)"
)
print(f"=======================================================")
print(
    f"Same-ID: {np.mean(same_sims):.4f} (+-{np.std(same_sims):.4f}) min: {np.min(same_sims):.4f}"
)
print(
    f"Diff-ID: {np.mean(diff_sims):.4f} (+-{np.std(diff_sims):.4f}) max: {np.max(diff_sims):.4f}"
)
...
if separation > 0.50 and fm_075 < 3:
    print("\nVERDICT: EXCELLENT")
elif separation > 0.40 and fm_075 < 5:
    print("\nVERDICT: GOOD")
...
for lbl in sorted(set(all_labels)):
    ...
    embs = torch.tensor(
        all_emb[mask]
    )  # Convert back to tensor just for clean norm wrapper if desired, np is fine
...
print(
    f"  Centroid high-confidence (gap > 0.20): {(centroid_gaps > 0.20).mean() * 100:.1f}%"
)
...
for label in unique_labels:
    ...
    print(
        f"  [{status:4s}] {name:25s}: mean={mean_sim:.4f} min={min_sim:.4f} (n={mask.sum()})"
    )

id_means = [
    (np.mean(sims), lbl) for lbl, sims in id_to_same_sims.items() if len(sims) > 0
]
...
for mean_sim, lbl in id_means[:10]:
    ...
    print(
        f"  [{len(sims_arr)} pairs] {identity:20s}: Mean={mean_sim:.4f} | Worst={worst:.4f}"
    )
...
cross_id_pairs = defaultdict(list)
for sim_val, i, j in worst_cross_pairs:
    lbl_i = all_labels[i]
    lbl_j = all_labels[j]
    pair = tuple(sorted([lbl_i, lbl_j]))
    cross_id_pairs[pair].append(sim_val)

for (lbl1, lbl2), sims in cross_id_pairs.items():
    if len(sims) >= 2 and sum(sims) / len(sims) > 0.80:
        name1 = full_eval_dataset.idx_to_class.get(lbl1, f"ID_{lbl1}").split("/")[-1]
        name2 = full_eval_dataset.idx_to_class.get(lbl2, f"ID_{lbl2}").split("/")[-1]
        print(
            f"  [FLAG for merge review] {name1} <-> {name2}: mean_sim={sum(sims) / len(sims):.4f} (n={len(sims)})"
        )
...
ax1.hist(
    ...
    alpha=0.6,
    color="green",
    label=f"Same ID (mean={np.mean(same_sims):.3f})",
    density=True,
)
ax1.hist(
    diff_sims,
    ...
    alpha=0.6,
    color="red",
    label=f"Diff ID (mean={np.mean(diff_sims):.3f})",
    density=True,
)
...
ax1.axvline(
    ...
    linestyle="--",
    linewidth=1,
    label="Old Threshold 0.65",
    alpha=0.6,
)
ax1.set_xlabel("Cosine Similarity", fontsize=12)
ax1.set_ylabel("Density", fontsize=12)
ax1.set_title(f"v8 Zero-Shot Separation (Gap = {separation:.4f})", fontsize=14)
...
# Re-embed eval set with flip TTA
...
save_dict = {
    ...
    "training_config": {
        "version": "v8.0_intra_class_collapse",
        "loss": "HardPosAlign(k=2, margin=0.7) + CenterLoss + ArcFace + GlobalBatchHardTriplet",
        "ms_alpha": 2.0,
        "ms_beta": 75.0,
        "ms_hard_neg_k": 20,
        "ms_temperature": 0.2,
        ...
        "augmentation": "v8_region_disrupting",
        "eval_split": "disjoint_80_20_zero_shot",
        "data_version": "training_heads_v6_cleaned_v85",
    },
}

torch.save(save_dict, "../models/elephant_head_reid_v8.1.pth")
shutil.copy2(
    "../models/elephant_head_reid_v8.1.pth",
    "../models/elephant_head_reid_v8_download.pth",
)

print("Model saved: elephant_head_reid_v8.pth")
print(f"  Version:            v8.0_intra_class_collapse")
print(f"  Separation gap:     {separation:.4f}")
print(f"  False merge @ 0.75: {fm_075:.2f}%")
print(f"  Download:           elephant_head_reid_v8_download.pth")
